...
export async function POST(req: NextRequest) {
  try {
    ...
    // Verstuur e-mail via Brevo (v2 SDK: BrevoClient)
        ...
        try {
          ...
          await prisma.emailLog.create({
            data: {
              ...
              onderwerp,
              status: 'VERZONDEN',
              providerRef: (result as any)?.messageId ?? null,
            }
          })
        } catch (mailErr: any) {
          await prisma.emailLog.create({
...
function buildEmailHtml(type: string, periodeStart: string, periodeEinde: string, s: any) {
  return `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#333;">
      ...
      <div style="background:#13161b;padding:28px 32px;border-radius:0 0 12px 12px;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr>
            <td style="padding:10px 14px;background:#1a1d24;color:#7a7f8e;font-size:12px;text-transform:uppercase;">Totale omzet</td>
