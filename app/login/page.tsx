...
type Stap = 'inloggen' | '2fa-totp' | '2fa-email'
...
export default function LoginPage() {
  ...
  // Refs voor stabiele focus — voorkomt re-render focus-verlies
  ...
  // Zet focus op het juiste veld bij stapwissel
  useEffect(() => {
    if (stap === 'inloggen')   emailRef.current?.focus()
    if (stap === '2fa-totp')   codeRef.current?.focus()
    if (stap === '2fa-email')  codeRef.current?.focus()
  }, [stap])
        ...
        <div className="card">
          ...
          {stap === '2fa-totp' && <>
            ...
            <form onSubmit={handleTwofa} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <button type="button" className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center' }} onClick={terugNaarLogin}>
                ← Terug naar inloggen
              </button>
            </form>
          </>}
          ...
          {stap === '2fa-email' && <>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>
              {emailVerzonden
                ? <>Code verstuurd naar <strong style={{ color: 'var(--text)' }}>{email}</strong>. Controleer je inbox.</>
                : 'Code versturen...'}
            </div>
            {error && <div className="alert alert-red" style={{ marginBottom: 16 }}>✕ {error}</div>}
            <form onSubmit={handleTwofa} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <button
                ...
                onClick={async () => { setTwofaCode(''); await stuurEmailCode() }}
              >
                ↻ Nieuwe code sturen
              </button>
              <button type="button" className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center' }} onClick={terugNaarLogin}>
