...
export default async function RapportagesPage() {
  const [rapportages, emailInstellingen] = await Promise.all([
    prisma.rapportage.findMany({
    ...
    prisma.emailInstelling.findMany({ orderBy: { aangemaaktOp: 'asc' } }),
  ])

  const statusBadge: Record<string, string> = {
    GEREED: 'badge-green', IN_WACHTRIJ: 'badge-amber', GENEREREN: 'badge-blue', MISLUKT: 'badge-red',
  }
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: 4, fontSize: 13 }}>Genereer, download en verstuur periodieke overzichten</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }}>
        ...
        <div className="card" style={{ padding: 0 }}>
          ...
          {rapportages.length === 0
            ? <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-dim)', fontSize: 13 }}>Nog geen rapportages</div>
                    ...
                    return (
                      <tr key={r.id}>
                        <td>
                          ...
                          {samenvatting?.omzet != null && <div style={{ fontSize: 10, color: 'var(--green)', fontFamily: 'DM Mono, monospace' }}>{formatEuro(samenvatting.omzet)}</div>}
                        </td>
        ...
        {/* Sidebar */}
        <RapportageForm emailInstellingen={emailInstellingen} />
      </div>
    </div>
  )
}
